...
my %sorters = (
   asc         => sub { $output_object{$b}{$opt_sort_by} <=> $output_object{$a}{$opt_sort_by} },
   desc        => sub { $output_object{$a}{$opt_sort_by} <=> $output_object{$b}{$opt_sort_by} },
...
foreach my $file (@ARGV) {
  ...
  if (! defined $FileStats->{min}) {
  	say STDERR "Fatal error: statistics not calculated parsing \"$file\". Aborting.";
...
sub version {
  ...
  Program to calculate N50 from multiple FASTA/FASTQ files, based on Proch::N50 $Proch::N50::VERSION
